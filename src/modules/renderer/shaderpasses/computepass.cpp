...
std::array<size_t, 6> ComputePass::ResourceCounts() const {
    return {{
        samplers.size(),
        readonly_storage_textures.size(),
        readonly_storage_buffers.size(),
        readwrite_storage_textures.size(),
        readwrite_storage_buffers.size(),
...
void ComputePass::Create() {
    if (!spirv || !spirv_size || spirv_size > std::numeric_limits<size_t>::max() / sizeof(*spirv))
    ...
    SDL_GPUComputePipeline* newPipeline = SDL_CreateGPUComputePipeline(device, &info);
...
void ComputePass::Execute(SDL_GPUCommandBuffer* cmd) {
    ...
    const bool indirect = indirect_dispatch_buffer != nullptr;

    glm::uvec3 groups(1, 1, 1);

    if (!indirect) {
        groups = dispatchFunc ? dispatchFunc(*this) : glm::uvec3(1, 1, 1);
        if (!groups.x || !groups.y || !groups.z) {
            return;
        }
    }

    SDL_GPUBuffer* indirectBufferGPU = nullptr;

    if (indirect)
        indirectBufferGPU = RequireGPU(indirect_dispatch_buffer);
    ...
    for (size_t i = 0; i < uniform_buffers.size(); ++i) {
        ...
        SDL_PushGPUComputeUniformData(cmd, static_cast<Uint32>(i), bytes.data(), static_cast<Uint32>(bytes.size()));
    }

    SDL_GPUComputePass* pass =
        SDL_BeginGPUComputePass(
            cmd,
            sdl_readwrite_storage_textures.empty()
                ? nullptr
                : sdl_readwrite_storage_textures.data(),
            static_cast<Uint32>(sdl_readwrite_storage_textures.size()),
            sdl_readwrite_storage_buffers.empty()
                ? nullptr
                : sdl_readwrite_storage_buffers.data(),
    ...
    if (!sdl_samplers.empty())
        SDL_BindGPUComputeSamplers(pass, 0, sdl_samplers.data(), static_cast<Uint32>(sdl_samplers.size()));

    if (!sdl_readonly_storage_textures.empty())
        SDL_BindGPUComputeStorageTextures(pass, 0, sdl_readonly_storage_textures.data(), static_cast<Uint32>(sdl_readonly_storage_textures.size()));

    if (!sdl_readonly_storage_buffers.empty())
        SDL_BindGPUComputeStorageBuffers(pass, 0, sdl_readonly_storage_buffers.data(), static_cast<Uint32>(sdl_readonly_storage_buffers.size()));


    if (indirect)
        SDL_DispatchGPUComputeIndirect(pass, indirectBufferGPU, indirect_dispatch_offset);
    else
